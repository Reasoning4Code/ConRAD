...
def lint_code(repo_playground, temp_name, code, prev_code="") -> tuple[bool, set, set]:
    ...
    with open(f"{repo_playground}/{temp_name}", "w" ,encoding='utf-8') as f:
        f.write(prev_code)
...
def fake_git_repo(repo_playground, file_pathes, old_contents, new_contents) -> str:
    ...
    res = subprocess.run(f"cd {repo_playground} && git init", shell=True)

    for file_path, old_content, new_content in zip(
        file_pathes, old_contents, new_contents
    ):

        full_file_path = Path(repo_playground) / file_path
        # create a file
        full_file_path.parent.mkdir(parents=True, exist_ok=True)
        print(f"[CMD] Created directory structure for {file_path}")
        # res1 = subprocess.run(
        #     f"mkdir -p {repo_playground}/{os.path.dirname(file_path)}", shell=True
        # )
        # print(f"[CMD] mkdir -p at function fake_git_repo")
        # print(f"[RET] {res1.returncode}")
        # print(f"[STDOUT]\n{res1.stdout}")
        # print(f"[STDERR]\n{res1.stderr}")
        ...
        print(f"[CMD] git add and commit for {file_path}")
        print(f"[ADD RET] {res1.returncode}")
        print(f"[COMMIT RET] {res2.returncode}")
...
def fake_git_apply(repo_playground, file_path, old_content, patch) -> str:
    ...
    # create a fake git repo
    res = subprocess.run(f"cd {repo_playground} && git init", shell=True, capture_output=True, text=True)
    print(f"[CMD] git init at function fake_git_apply")
    ...
    with open(f"{repo_playground}/{file_path}", "w" ,encoding='utf-8') as f:
        f.write(old_content)
    ...
    with open(f"{repo_playground}/{patch_file}", "w" ,encoding='utf-8') as f:
        f.write(patch)
    o = subprocess.run(
...
def fake_git_apply_multiple(repo_playground, file_path_contents, patch) -> dict:
    ...
    # create a fake git repo
    res = subprocess.run(f"cd {repo_playground} && git init", shell=True, capture_output=True, text=True)
    print(f"[CMD] git init at function fake_git_apply_multiple")
    print(f"[RET] {res.returncode}")
    print(f"[STDOUT]\n{res.stdout}")
    print(f"[STDERR]\n{res.stderr}")
...
""".strip()
    ...
    from util.preprocess_data import line_wrap_content
